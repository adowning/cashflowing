...
import {
  ...
  PendingEvent,
} from "@/rest/services/dbupdates/types";
import PgRealtimeClient from "@/rest/services/update.service";
...
export class RealtimeService {
  ...
  private _registerPgClientErrorHandlers(): void {
    ...
    if (this.pgClient?.pool) {
      ...
      console.warn(
        "[RealtimeService] Cannot register pool error handler - pool not ready."
      );
    }
  }
  ...
  public async startListening(): Promise<void> {
    ...
    if (!this.isServerSet) {
      console.error(
        "[RealtimeService] Cannot start listening: Server instance not set."
      );
      throw new Error(
    ...
    try {
      console.log(
        "[RealtimeService] Connecting and setting up DB listeners..."
      );
  ...
  private _handleRawDbEvent(event: PendingEvent): void {
    ...
    let updatePayload: z.infer<typeof DatabaseUpdate.shape.payload> | undefined;
    ...
    switch (event.table) {
      ...
      default:
        console.warn(
          `[RealtimeService] No userId extraction logic for table: ${event.table}`
        );
        return;
    }
    ...
    if (!userId) {
      console.warn(
        `[RealtimeService] Could not determine userId for event:`,
        event
      );
      return;
    }
  ...
  private publishDbUpdate(
    ...
    if (!validationResult.success) {
      console.error(
        `[RealtimeService]z Failed to validate DATABASE_UPDATE for userId ${userId}:`,
