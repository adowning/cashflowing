...
export async function updateUserEmail(req: HonoRequest, user: Partial<User>) {
  ...
  try {
    ...
    if (existingUser && existingUser.id !== user.id) {
      return new Response(
        JSON.stringify({ message: 'Email already in use', code: 400 }),
        { status: 400 },
      )
    }
...
export async function updateUserCashtag(req: HonoRequest, user: Partial<User>) {
  ...
  try {
    ...
    if (existingUser && existingUser.id !== user.id) {
      return new Response(
        JSON.stringify({ message: 'cashtag already in use', code: 400 }),
        { status: 400 },
      )
    }
...
export async function suspendUser(req: HonoRequest, user: Partial<User>) {
  ...
  try {
    ...
    return new Response(
      JSON.stringify({ code: 200, message: 'User suspended successfully' }),
    )
  } catch (error: any) {
...
export async function checkUser(req: HonoRequest) {
  ...
  return new Response(
    JSON.stringify({ code: 200, message: 'User check successful' }),
  )
}
